    ...
    <div className="max-w-[1400px] mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-white/5 pb-10">
        <div>
            ...
            <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.5em] mt-2">Archives of relay transmissions</p>
        </div>
          ...
          {(['ALL', ...Object.values(RequestStatus)] as const).map((s) => (
            <button
              ...
              className={`px-6 py-2 rounded-xl text-[10px] font-black italic transition-all ${
                filter === s ? 'bg-[#00D1FF] text-black shadow-[0_0_15px_rgba(0,209,255,0.4)]' : 'text-slate-400 hover:text-white'
              }`}
            >
              {s}
            </button>
      ...
      <div className="space-y-4">
          ...
          <div key={req.id} className="premium-panel light-sweep p-8 flex flex-col lg:flex-row items-center justify-between group relative overflow-hidden gap-8">
             ...
             <div className="flex items-center gap-8 w-full lg:w-auto">
                ...
                <div className="flex items-center gap-6 border-l border-white/5 pl-8">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-300 tracking-widest mt-1 uppercase">ID: {req.id.toUpperCase()} // ATTACHED: {new Date(req.createdAt).toLocaleDateString()}</p>
                    </div>
                </div>
             </div>
             ...
             <div className="flex items-center justify-between lg:justify-end gap-12 w-full lg:w-auto">
                <div className="text-right">
                    ...
                    <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Transaction Weight</p>
                </div>
                
                <div className="flex gap-3">
                  <button 
                    onClick={() => handleCopyLink(req.id)}
                    className="w-12 h-12 flex items-center justify-center rounded-xl bg-white/5 border border-white/10 text-slate-300 hover:text-[#00D1FF] transition-all group"
                  ...
                  {req.status === RequestStatus.PENDING && (
                    <button 
                      ...
                      className="w-12 h-12 flex items-center justify-center rounded-xl bg-red-500/5 border border-red-500/10 text-slate-400 hover:text-red-500 hover:border-red-500/40 transition-all"
        ...
        {filteredRequests.length === 0 && (
          <div className="p-32 text-center space-y-8 bg-white/2 rounded-3xl border border-dashed border-white/5">
            <div className="w-24 h-24 bg-white/5 rounded-full flex items-center justify-center mx-auto text-3xl text-slate-600">
                <i className="fa-solid fa-radar"></i>
            </div>
            <p className="text-slate-400 font-black italic uppercase tracking-[0.4em]">No signatures matching frequency</p>
          </div>
        )}
      </div>
    </div>
...
const StatusIcon: React.FC<{status: RequestStatus}> = ({ status }) => {
    const icons = {
        ...
        [RequestStatus.EXPIRED]: { icon: 'fa-ban', color: 'text-slate-400 bg-slate-950 border-white/10' },
